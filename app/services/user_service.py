...
class UserService:
    ...
    async def create_user(self, db: AsyncSession, user_data: UserCreateRequest) -> tuple[Optional[UserResponse], Optional[ErrorResponse]]:
        """
        새로운 사용자를 생성합니다.
    ...
    async def get_user_by_id(self, db: AsyncSession, user_id: int) -> tuple[Optional[UserResponse], Optional[ErrorResponse]]:
        """
        ID로 사용자를 조회합니다.
    ...
    async def get_all_users(self, db: AsyncSession, skip: int = 0, limit: int = 100) -> tuple[Optional[UserListResponse], Optional[ErrorResponse]]:
        """
        모든 사용자를 조회합니다.
        ...
        try:
            ...
            # 전체 사용자 수를 구하려면 별도 쿼리가 필요하지만,
            # 여기서는 간단히 현재 조회된 수를 사용
    ...
    async def update_user(self, db: AsyncSession, user_id: int, update_data: UserUpdateRequest) -> tuple[Optional[UserResponse], Optional[ErrorResponse]]:
        """
        사용자 정보를 수정합니다.
        ...
        try:
            ...
            if "email" in update_dict:
                existing_user = await self.user_repository.get_by_email(db, update_dict["email"])
